...
describe('z.tracking.EventTrackingRepository', () => {
  ...
  beforeEach(() => {
    return new TestFactory().exposeTrackingActors().then(({repository}) => {
      trackingRepository = repository.tracking;
      trackingRepository._isDomainAllowedForTracking = () => true;
    });
  });

  describe('Initialization', () => {
    it('enables error reporting, user tracking and subscribes to tracking events', () => {
      expect(trackingRepository.providerAPI).toBeUndefined();
      trackingRepository.providerAPI = true;
      spyOn(trackingRepository, '_enableErrorReporting').and.callThrough();
      spyOn(trackingRepository, '_initTracking').and.callThrough();
      spyOn(trackingRepository, '_subscribeToTrackingEvents').and.callThrough();
      ...
      return trackingRepository.init(true).then(() => {
        expect(trackingRepository.providerAPI).toBeDefined();
        expect(trackingRepository._enableErrorReporting).toHaveBeenCalled();
        expect(trackingRepository._initTracking).toHaveBeenCalled();
        expect(trackingRepository._subscribeToTrackingEvents).toHaveBeenCalled();
      });
    });
    ...
    it('allows changing initial tracking properties', () => {
      ...
      expect(trackingRepository.isErrorReportingActivated).toBe(false);
      expect(trackingRepository.isUserAnalyticsActivated).toBe(false);
      trackingRepository._trackEvent = jasmine.createSpy('_trackEvent');

      return trackingRepository.init(true).then(() => {
        expect(trackingRepository.isErrorReportingActivated).toBe(true);
        expect(trackingRepository.isUserAnalyticsActivated).toBe(true);
        amplify.publish(z.event.WebApp.ANALYTICS.EVENT, 'i_am_an_event');

        expect(trackingRepository._trackEvent).toHaveBeenCalledTimes(1);
        amplify.publish(z.event.WebApp.ANALYTICS.EVENT, 'i_am_another_event');

        expect(trackingRepository._trackEvent).toHaveBeenCalledTimes(2);
        amplify.publish(z.event.WebApp.PROPERTIES.UPDATE.PRIVACY, false);

        expect(trackingRepository._trackEvent).toHaveBeenCalledTimes(3);
  ...
  describe('User Tracking', () => {
    beforeEach(() => {
      trackingRepository.providerAPI = true;
      trackingRepository._trackEvent = jasmine.createSpy('_trackEvent');
      return trackingRepository.init(true);
    });

    it('immediately reports events', () => {
      amplify.publish(z.event.WebApp.ANALYTICS.EVENT, 'i_am_an_event');

      expect(trackingRepository._trackEvent).toHaveBeenCalled();
      expect(trackingRepository._trackEvent).toHaveBeenCalledTimes(1);
    });
    ...
    it('allows additional parameters for events', () => {
      ...
      expect(trackingRepository._trackEvent).toHaveBeenCalledWith(event_name, attributes);
    });
  });
  ...
  describe('Error Tracking', () => {
    ...
    it('does not log the same error twice', () => {
      ...
      let error_payload = trackingRepository._checkErrorPayload(raygun_payload);

      expect(error_payload).toBe(raygun_payload);

      _.times(100, () => (error_payload = trackingRepository._checkErrorPayload(raygun_payload)));
      ...
      error_payload = trackingRepository._checkErrorPayload(raygun_payload);
